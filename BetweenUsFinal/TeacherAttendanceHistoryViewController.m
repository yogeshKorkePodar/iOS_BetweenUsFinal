//
//  TeacherAttendanceHistoryViewController.m
//  BetweenUs
//
...
@interface TeacherAttendanceHistoryViewController ()
{
    NSDictionary *newDatasetinfoTeacherAttendanceHistory;
}
@end

@implementation TeacherAttendanceHistoryViewController
@synthesize month;

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    NSLog(@"<<<< Inside TeacherAttendanceHistoryViewController >>>>");
    ...
    month = month;
    DeviceType= @"IOS";
    self.teacherAttendanceHistoryTableView.dataSource = self;
    self.teacherAttendanceHistoryTableView.delegate = self;
...
-(void)webserviceCall{
    NSString *urlString = app_url @"PodarApp.svc/ViewStudAttendDetails";
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(!responseData == nil){
                ...
                [hud hideAnimated:YES];
                [self.teacherAttendanceHistoryTableView reloadData];
            }
        });
    });
