...
-(void) httpPostRequest{
    ...
    if(firstTime == YES){
        NSError *err;
        NSString *str = app_url @"UpdateMessageReadStatus";
        ...
        NSError *err;
        NSString *str = app_url @"ReplyToMessage";
        str = [str stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        NSURL *url = [NSURL URLWithString:str];
...
- (IBAction)replyButton:(id)sender {
    ...
    if(_enterMessage_textfield.text.length>0){
        NSString *urlString = app_url @"ReplyToStudentMessage";
        //Pass The String to server
...
-(void) uploadFilewithData:(NSMutableArray *)fileByteArray{
    //Pass The String to server
    NSString *urlString = app_url @"UploadAdminMsgAttachment";
