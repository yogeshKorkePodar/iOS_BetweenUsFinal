//
//  TeacherSmsStudentListViewController.m
//  BetweenUs
//
...
@interface TeacherSmsStudentListViewController ()
{
    UIButton *btn;
    NSIndexPath *path;
    NSInteger *cellRow;
...
@implementation TeacherSmsStudentListViewController
...
- (void)viewDidLoad {
    ...
    [_closeClick addGestureRecognizer:tapGestCloseImage];
...
-(void)webserviceCall{
    if(firstTime==YES){
        NSString *urlString = app_url @"GetTeacherSMSStudentList";
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(!responseData==nil){
                    ...
                    teacherStudentArray = [receivedData objectForKey:@"SmsStudentResult"];
...
- (IBAction)sentSMSToselectedStudent:(id)sender {
    if(checkboxAllClicked ==NO && individualButtonClicked == NO) {
        ...
        TeacherSendSMSViewController *teacherSendSMSViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"TeacherSendSMS"];
        teacherSendSMSViewController.stu_id = stu_id;
        
        [self.navigationController pushViewController:teacherSendSMSViewController animated:YES];
    }
